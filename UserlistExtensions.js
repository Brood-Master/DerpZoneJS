/*!
 **|  CyTube Userlist Extensions
 **|  Copyright Xaekai 2014 - 2017
 **|  Version 2018.01.08.1818
 **|
 **@preserve
 */
"use strict";
...
function userlist(justList) {
    var users = Array.prototype.slice.call(document.querySelector("#userlist").children).filter(function(cv) {
    ...
    const options = Object.assign({}, {
        autoHider: false,
        scrutinize: false,
        trustedHosts: new RegExp(["imgur.com", "pink.horse", "derpibooru.org", "sli.mg"].join("|").replace(/\./g, "\\.")),
        additionalHosts: new RegExp(["some.website"].join("|").replace(/\./g, "\\."))
    }, window[CHANNEL.name].modulesOptions ? window[CHANNEL.name].modulesOptions.userlist : undefined);
    ...
    function trustAvatar(data) {
        ...
        return trustedHosts.test(host + "." + tld) || additionalHosts.test(host + "." + tld)
    }
    ...
    function() {
        ...
        for (var i = users.length - 1; i >= 0; i--) {
            findUserlistItem(users[i]).find("span:contains(" + users[i] + ")").addClass("userlist-" + users[i]).parent().attr("id", "useritem-" + users[i]);
            if (findUserlistItem(users[i]).data()["profile"]) {
                findUserlistItem(users[i]).data()["profile"]["image"] = trustAvatar(findUserlistItem(users[i]).data()) ? findUserlistItem(users[i]).data()["profile"]["image"] : null
            }
        }
    }();
    ({
        ...
        initialize: function() {
            ...
            if (localStorage.getItem(`${CHANNEL.name}_autohiderToggle`) !== null) {
                this.toggleState = parseInt(localStorage.getItem(`${CHANNEL.name}_autohiderToggle`))
            }
            if (localStorage.getItem(`${CHANNEL.name}_autohiderTimeout`) !== null) {
                this.hideAfter = parseInt(localStorage.getItem(`${CHANNEL.name}_autohiderTimeout`))
            }
            if (this.toggleState) {
        ...
        toggle: function() {
            ...
            localStorage.setItem(`${CHANNEL.name}_autohiderToggle`, this.toggleState ? 0 : 1);
        ...
        createCustomSettings: function() {
            ...
            $("<div>").addClass("form-group").append($("<button/>").prop("id", "ccs-userlisthide_toggle").addClass("btn btn-sm col-sm-2").addClass(this.toggleState ? "btn-warning" : "btn-default").attr("title", "Toggle Autohide").html("&nbsp;Toggle").prepend($("<span>").addClass("glyphicon").addClass(this.toggleState ? "glyphicon-eye-close" : "glyphicon-eye-open")).click(this.toggle.bind(this))).append($("<label>").addClass("control-label col-sm-2").attr("for", "ccs-userlisthide_timeout").prepend($("<span>").addClass("label label-info pull-right").text("Timeout:"))).append($("<div>").addClass("col-sm-8").append($("<input>").prop("id", "ccs-userlisthide_timeout").addClass("form-control cs-textbox").attr("type", "text").attr("placeholder", "Delay before hiding in ms. Default is " + this.hideAfter).val(this.hideAfter !== 30 * 1e3 ? this.hideAfter : null).keyup(function() {
                var box = $(this);
                var value = box.val();
                ...
                setTimeout(function() {
                    ...
                    if (parseInt(value) > -1) {
                        if (parseInt(value) < 1e4) {
                            box.val(1e4);
                            localStorage.setItem(`${CHANNEL.name}_autohiderTimeout`, 1e4);
                            console.log("Userlist autohiding timeout set to minimum value")
                        } else {
                            localStorage.setItem(`${CHANNEL.name}_autohiderTimeout`, parseInt(value));
