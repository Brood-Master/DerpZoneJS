...
(function(CyTube_Whispers) {
    ...
    const options = Object.assign({}, {
        ...
        cleanTick: 1e3,
        cleanMaxAge: 45 * 1e3
    }, CLIENT.modules.options ? CLIENT.modules.options.whispers : undefined);
    void {
        timeOffset: 0,
        ...
        start: function() {
            ...
            setInterval((tickValue => {
                this.cleanScan(tickValue)
            }), this.cleanTick, this.cleanTick)
        },
        ...
        checkOldJoins: function(name) {
            const matches = $("div.chat-msg-\\\\\\$server\\\\\\$").filter(((ind, cv) => cv.innerText.match(`${name} joined`)));
            if (matches.length > 1) {
                matches.first().remove()
            }
        },
        checkOldLeaves: function(name) {
            const matches = $("div.chat-msg-\\\\\\$server\\\\\\$").filter(((ind, cv) => cv.innerText.match(`${name} disconnected`)));
            if (matches.length > 1) {
                matches.first().remove()
            }
        },
