...
(function(CyTube_ColorMap) {
    ...
    $("#colorFilters").remove();
    if (!$("#chatinput").length) {
        $("#chatline").wrap('<div id="chatinput" class="input-group" style="width:100%">')
    }
    ...
    var inputButton = $("<button/>").addClass("btn btn-default dropdown-toggle").attr("type", "button").attr("href", "javascript:void(0)").attr("data-toggle", "dropdown").html("<span class='dropLabel' id='colorFiltersDropText'>Colors </span><span class='caret'></span>").appendTo(inputGroup);
    var colormapDropup = generateColorTable().addClass("dropdown-menu").attr("role", "menu").appendTo(inputGroup);
    $("<button>").addClass("btn btn-warning btn-xs").append($("<span>").addClass("glyphicon glyphicon-resize-full")).on("click", function(event) {
        $("#chatline").trigger({
            type: "fontSize",
            change: "up"
        });
        return event.stopPropagation()
    }).css("position", "absolute").css("right", "0").css("bottom", "0").css("font-family", "Equestria").css("border-radius", "2px").css("margin", "4px").css("padding-top", "4px").appendTo("#colorFiltersPopup");
    $("<button>").addClass("btn btn-warning btn-xs").append($("<span>").addClass("glyphicon glyphicon-resize-small")).on("click", function(event) {
        $("#chatline").trigger({
            type: "fontSize",
            change: "down"
        });
        return event.stopPropagation()
    }).css("position", "absolute").css("left", "0").css("bottom", "0").css("font-family", "Equestria").css("border-radius", "2px").css("margin", "4px").css("padding-top", "4px").appendTo("#colorFiltersPopup");

    function generateColorTable() {
        function generateMapArea(data) {
            return $("<area>").attr("alt", "#" + data.hex).attr("coords", data.coords).attr("onclick", "colormapClick(event,this)").attr("onmouseover", "colormapMouseover(this)").attr("shape", "poly").css("cursor", "pointer")
        }
        ...
        var image = $("<img>").attr("alt", "colormap").attr("src", "https://cdn.discordapp.com/attachments/701588063616499792/706984696806113421/colormap.gif").attr("usemap", "#colormap").css("margin-right", "2px").appendTo(td);
