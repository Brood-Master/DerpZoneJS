...
(function(CyTube_HotKeys) {
    ...
    $(document).on("keydown.hotkeys", function(event) {
        if (!event.shiftKey && !event.ctrlKey && event.altKey) {
            if (event.which === KEYCODE.W && $("#cinematoggle").length && !(localStorage.getItem(`BrazilianKeyboard`) === "true")) {
                $("#cinematoggle").click();
            ...
            if (event.which === KEYCODE.S) {
                ...
                $("#messagebuffer").trigger("whisper", `[System] Your spoiler setting is now: ${$("#messagebuffer").data("spoilertype")}.`);
                event.preventDefault();
                return false
            }
        }
        if (!event.ctrlKey) return true;
        if (typeof event.target.selectionStart == "undefined" || event.target.selectionStart == null) return true;
        var tag = {
            wrap: false,
            braced: false
        };
        ...
        if (zero) {
            caret = end + tag.code.length + function() {
    ...
    $("#messagebuffer").on("click.quickact", "strong.username", function(e) {
        var user = $(this).parent().parent().attr("class").match(/chat-msg-(\w+)/)[1];
        if (e.altKey && e.shiftKey && CLIENT.rank > 2) {
            if (userlist()[user]) {
                if (CLIENT.rank > userlist()[user].rank) {
                    socket.emit("chatMsg", {
                        msg: "/kick " + user + " Quick Kick"
                    })
                }
            }
        }
    ...
    $("#chatline").on("paste.enhanced", function(ev) {
        ...
        text = shiftKey && /^https?:\/\/(?:\w+\.)*(?:\w+\.)(?:\w+)\/(?:\w+\/)*(?:[^ ]+)(?:png|gif|jpe?g)(?:[^ ]+)?$/.test(text) ? text + ".pic" : text;
        var success = document.execCommand("insertText", false, text);
        if (success) {
            ev.preventDefault()
        }
        if (escCode) {
            $("#chatline").trigger(function() {
